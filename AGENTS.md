...
- **Consequential Deploy Interception**: `scripts/git-guardrail.sh` also intercepts `vercel deploy --prebuilt`, `vercel deploy --prod`, `vercel env rm`, `vercel domains rm` and `prisma migrate deploy`. `--prebuilt` is the sharpest: it ships a locally built artifact without rebuilding, so a build whose data source was unreachable deploys fallback content over real content. These guards are skipped when `CI=true`, where deliberate automation carries its own verification (production itself ships only through Vercel's build of `main`, ADR 0049), and share the single `ALLOW_DANGEROUS_GIT=1` override rather than introducing a variable per command.
...
- **Standard Developer Workflow**: `main` is the sole long-lived integration, release, and Vercel production branch. Cut short-lived prefixed feature and agent branches from `main`, then squash-merge their pull requests into `main` after the required CI gates pass. The current `dev` branch and its `dev` → `main` PR are a one-time Jules consolidation exception recorded in [ADR 0050](adr/0050-jules-consolidation-release.md); do not route new feature work into `dev`. Any `main` updates needed to finish this release are merged into `dev` without rewriting shared history. CI runs for pull requests targeting `main` and on `main` pushes. Keep automatic non-production Vercel deployments disabled unless an operator deliberately requests one preview. A merge to `main` deploys production through Vercel alone (ADR 0049): Vercel's Git integration builds `main`, `scripts/build.js` applies migrations first on Vercel production builds only, and Vercel's Deployment Checks hold the domains until CI's Merge Gate passes. GitHub Actions runs CI and never deploys, so production secrets live only in Vercel. Every migration ships on merge, before CI on `main` finishes, and must be expand/contract. The Vercel dashboard's Redeploy rebuilds the commit it is pressed on and is never a way to ship `main`.
