...
- **Consequential Deploy Interception**: `scripts/git-guardrail.sh` also intercepts `vercel deploy --prebuilt`, `vercel deploy --prod`, `vercel env rm`, `vercel domains rm` and `prisma migrate deploy`. `--prebuilt` is the sharpest: it ships a locally built artifact without rebuilding, so a build whose data source was unreachable deploys fallback content over real content. These guards are skipped when `CI=true`, leaving `release.yml`'s deliberate build-once-promote-same-artifact path (ADR 0038) unimpeded, and share the single `ALLOW_DANGEROUS_GIT=1` override rather than introducing a variable per command.
