...
Both are additive and backward compatible with the previously deployed
application, which does not reference either table.
...
- Reaction write-buffer durability: buffered reactions now drain to Postgres on
  the daily maintenance pass instead of accumulating in Redis unbounded.
- Reaction flush idempotency: a replayed batch can no longer double-insert rows
  or double-decrement the buffer.
...
- CI typechecks with the compiler pinned in `package-lock.json` rather than the
  runner image's global TypeScript.
...
- CI builds through `npm run build`, the entrypoint Vercel runs, which pins
  `next build --webpack`. `npx next build` defaults to Turbopack on Next 16 and
  never completes for this application; it was cancelled at 41m47s.
