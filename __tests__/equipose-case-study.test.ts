...
describe("Equipose Randomization Case Study Integration & Security Invariants", () => {
  it("includes equipose-randomization in static FALLBACK_CASE_STUDIES array", () => {
    ...
    expect(study?.github_url).toBe("https://github.com/fderuiter/equipose-randomization");
  });
  ...
  it("registers canonical benchmark route and SEO metadata config", () => {
    const canonicalRoute = CANONICAL_ROUTES.find((r) => r.path === "/case-studies/equipose-randomization");
    ...
    expect(seoConfig.title).toContain("Equipose Randomization");
