...
describe("NeuroRecon: Accessibility & Keyboard Interactions Suite", () => {
  ...
  it("toggles View Modes (Split, 3D Only, 2D Only)", async () => {
    ...
    const view3DButton = buttons.find((btn) => btn.textContent?.includes("3D Only"));
    const view2DButton = buttons.find((btn) => btn.textContent?.includes("2D Only"));
    const viewSplitButton = buttons.find((btn) => btn.textContent?.includes("Split View"));
  ...
  it("adjusts brush radius when Paint or Erase mode is active", async () => {
    ...
    // Switch to Paint
    const paintButton = Array.from(container.querySelectorAll("button")).find((btn) =>
      btn.textContent?.includes("Voxel Paint") || btn.title?.includes("Voxel Paint")
    );
    await act(async () => {
      paintButton?.click();
    });
  ...
  it("triggers recon-all execution and updates telemetry state", async () => {
    ...
    const reconButton = Array.from(container.querySelectorAll("button")).find((btn) =>
      btn.textContent?.includes("RUN RECON-ALL")
    );
