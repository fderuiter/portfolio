...
describe('NeuroRecon 3D Asset Loader (loadExternalBrainMesh)', () => {
  ...
  it('loads and normalizes an .obj model successfully with MeshStandardMaterial', async () => {
    ...
    const mockLoad = vi.fn((url: string, onLoad: (res: THREE.Group) => void) => {
      onLoad(mockGroup);
    });

    (OBJLoader as unknown as ReturnType<typeof vi.fn>).mockImplementation(function (this: any) {
      this.load = mockLoad;
    });

    const result = await loadExternalBrainMesh('/models/brain-scan-test.obj', 'pial', 'both');

    expect(result).toBeInstanceOf(THREE.Group);
    expect(mockLoad).toHaveBeenCalledTimes(1);
    expect(computeNormalsSpy).toHaveBeenCalled();
    expect(mesh.material).toBeInstanceOf(THREE.MeshStandardMaterial);
    expect(((mesh.material as unknown) as THREE.MeshStandardMaterial).color.getHex()).toBe(0x93c5fd);
  ...
  it('loads and normalizes a .gltf / .glb model successfully', async () => {
    ...
    const mockLoad = vi.fn((url: string, onLoad: (res: { scene: THREE.Group }) => void) => {
      onLoad({ scene: mockScene });
    });

    (GLTFLoader as unknown as ReturnType<typeof vi.fn>).mockImplementation(function (this: any) {
      this.load = mockLoad;
    });

    const result = await loadExternalBrainMesh('/models/brain-scan-test.glb', 'inflated', 'lh');
  ...
  it('returns cached mesh on subsequent calls with identical arguments without re-fetching', async () => {
    ...
    const first = await loadExternalBrainMesh('/models/cached-test.glb', 'white', 'rh');
    const second = await loadExternalBrainMesh('/models/cached-test.glb', 'white', 'rh');
  ...
  it('gracefully falls back to procedural cortical mesh if loader fails or throws', async () => {
    ...
    const mockLoad = vi.fn((url: string, onLoad: any, onProgress: any, onError: (err: Error) => void) => {
    ...
    const fallbackMesh = await loadExternalBrainMesh('/models/corrupt-scan.glb', 'inflated', 'both');
  ...
  it('handles empty geometries and zero bounding box dimension without division by zero', async () => {
    ...
    const result = await loadExternalBrainMesh('/models/empty-scan.obj', 'pial', 'both');
    expect(result).toBeInstanceOf(THREE.Group);
