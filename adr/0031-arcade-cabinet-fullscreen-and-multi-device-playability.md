...
- **Caveat on the "iPhone 12 Mobile Safari" claim above**: Playwright's "Mobile Safari" project emulates the iPhone 12 viewport on the generic WebKit engine, which grants `requestFullscreen` on arbitrary elements — a capability real iPhone Safari does not have (it only supports native fullscreen on `<video>`, via a separate API this codebase doesn't use). That project therefore cannot confirm which branch — native or CSS-overlay fallback — a real device takes; it only confirms the layout is correct in whichever branch runs. `hooks/useFullscreen.ts`'s `isSupported` flag and the deliberate API-deletion tests in `__tests__/fullscreen-hook.test.tsx` and `__tests__/e2e/arcade-responsive-playability.spec.ts` are what actually exercise the real-iPhone (fallback) branch in CI; real-device spot-checking remains the only way to fully confirm this claim.
- When a cabinet is running the CSS-overlay fallback instead of the real Fullscreen API, `FullscreenButton` appends "(Compatibility Mode)" to its exit label/title so the difference is discoverable rather than silent.
