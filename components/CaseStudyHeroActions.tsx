...
export function CaseStudyHeroActions({
  ...
  const isKaggle =
    ...
    externalPlatformUrl?.includes("kaggle.com");

  const registryEntry = getSimulatorForCaseStudy(slug);
  const resolvedInteractiveUrl =
    interactiveUrl || registryEntry?.simulatorRoute;
  const resolvedInteractiveLabel =
    interactiveLabel && interactiveLabel !== "Launch Interactive Studio"
      ? interactiveLabel
      : registryEntry?.ctaLabel || "Launch Simulator";
    ...
    <div className="flex flex-wrap items-center gap-3 my-6">
      ...
      {githubUrl && (
        ...
        >
          ...
          {stats &&
            typeof stats.commitsCount === "number" &&
      ...
      {externalPlatformUrl && !isKaggle && (
        ...
        >
          ...
          <span className="text-xs md:text-sm font-semibold">
            {externalPlatformType
              ? `View on ${externalPlatformType.toUpperCase()}`
              : "External Resource"}
          </span>
        </a>
      )}
      ...
      {resolvedInteractiveUrl && (
        <Link
          href={resolvedInteractiveUrl}
          aria-label={resolvedInteractiveLabel}
          className="inline-flex items-center gap-2 min-h-[44px] px-4 py-2 rounded-lg bg-brand-cyan/10 hover:bg-brand-cyan/20 border border-brand-cyan/40 hover:border-brand-cyan text-brand-cyan text-sm font-semibold transition-all shadow-[0_0_15px_rgba(6,182,212,0.15)] focus:outline-none focus:ring-2 focus:ring-brand-cyan/60 active:scale-[0.98]"
        >
          <IconPlayerPlay className="w-4 h-4 text-brand-cyan fill-brand-cyan/20" />
          <span className="text-xs md:text-sm">{resolvedInteractiveLabel}</span>
        </Link>
      )}
    </div>
  );
