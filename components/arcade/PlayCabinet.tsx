...
import { IconPlayerPlay, IconPower, IconTerminal, IconAdjustmentsHorizontal } from "@tabler/icons-react";
...
const PlayCabinetInner: React.FC<PlayCabinetProps> = ({
  title,
  subtitle,
  accentColor,
  icon,
  ...
  const { isWizardOpen, openWizard, closeWizard, difficulty, audio } = useCabinet();
        ...
        <div className="mt-4 flex items-center justify-between w-full border border-zinc-800 bg-zinc-900/60 rounded-2xl px-4 py-2 font-mono text-xs text-zinc-500">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="uppercase tracking-wider font-bold text-zinc-300">Cabinet Engaged</span>
            </div>
            <span className="text-zinc-700">|</span>
            <span className="text-[10px] text-zinc-400 uppercase tracking-wider">
              Diff: <span className="text-amber-400 font-bold">{difficulty}</span>
            </span>
            <span className="text-zinc-700">|</span>
            <span className="text-[10px] text-zinc-400 uppercase tracking-wider">
              Vol: <span className="text-emerald-400 font-bold">{audio.muted ? "Muted" : `${Math.round(audio.volume * 100)}%`}</span>
            </span>
          </div>
        ...
        {isWarmingUp ? (
          ...
          <div className="flex-1 flex flex-col justify-between font-mono text-xs text-emerald-400 z-10 p-2">
            <div className="space-y-1.5 uppercase tracking-wide">
              ...
              <div>
                &gt; INITIALIZING PORT CLIENT... <span className="text-emerald-500">OK</span>
              </div>
              <div>
                &gt; ISOLATING ENGINE MEMORY... <span className="text-emerald-500">COMPLETE</span>
              </div>
              <div>
                &gt; CONSTRUCTING AUDIO GRAPH... <span className="text-emerald-500">READY</span>
              </div>
              <div>
                &gt; PRESET DIFFICULTY: <span className="text-amber-400">{difficulty.toUpperCase()}</span>
              </div>
              {bootProgress > 50 && (
                <div>
                  &gt; ESTABLISHING BUFFER CANVAS PORT... <span className="text-emerald-500">STABLE</span>
                </div>
              )}
              {bootProgress > 75 && (
                <div>
                  &gt; DYNAMICALLY MOUNTING GAME CHUNK... <span className="text-emerald-500">MOUNTED</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between border-b border-zinc-800/80 pb-4 font-mono text-[10px] text-zinc-500 z-10">
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${
                    isPrefetched ? "bg-amber-400" : "bg-zinc-600"
                  } animate-pulse`}
                />
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={openWizard}
                  className="px-2.5 py-1 bg-zinc-900/80 hover:bg-zinc-800 text-zinc-300 rounded-md border border-zinc-700/80 transition-colors flex items-center gap-1.5 text-[9px] uppercase tracking-wider font-bold"
                >
                  <IconAdjustmentsHorizontal className="w-3 h-3 text-amber-400" />
                  <span>Presets: <span className="text-amber-300">{difficulty}</span></span>
                </button>
                <span className="uppercase tracking-widest text-[9px] text-zinc-600 hidden sm:inline">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center pt-4 border-t border-zinc-800/60 z-10">
              ...
              <div className="flex justify-end w-full">
                <button
                  ...
                  onClick={handleOpenLaunchWizard}
...
export const PlayCabinet: React.FC<PlayCabinetProps> = (props) => {
  return (
    <CabinetProvider>
      <PlayCabinetInner {...props} />
    </CabinetProvider>
  );
};
