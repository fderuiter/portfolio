...
export const GAME_LOADOUTS: Record<string, GameLoadoutOption[]> = {
  "working-with-duck": [
    ...
    { id: "developer", name: "Pro Desk & Organic Bones", description: "Extended coding stamina & treaties for high-pressure sprints", badge: "Endurance" },
  ],
  "laser-loon": [
    { id: "ruby", name: "Thermal Ruby Ray", description: "High-temperature continuous laser beam for precise flag blasting", badge: "Thermal" },
    { id: "cryo", name: "Glacial Cryo-Shatter", description: "Freezes red tape and shatters rival flag obstacles instantly", badge: "Cryo" },
    { id: "tremolo", name: "Loon Tremolo Overdrive", description: "Sonic shockwave pulses with rapid recharge speed", badge: "Sonic" },
  ],
  "quasi-puzzler": [
    { id: "standard", name: "Standard Lean Tactics", description: "Introductory AST tactic inference and goal decomposition set", badge: "Standard" },
    { id: "solver", name: "Advanced Formal Solver", description: "Automated goal simplification and tactic recommendations", badge: "Automation" },
    { id: "axiom", name: "Lean 4 Axiom Pack", description: "Full mathematical proof tree suite with expanded hypothesis stack", badge: "Axiomatic" },
  ],
  "garmin-watch": [
    { id: "standard-ram", name: "32KB Standard Heap", description: "Balanced Monkey C memory allocation for smooth GC cycles", badge: "32KB RAM" },
    { id: "low-power", name: "Low-Power Runner", description: "Optimized battery life & reduced GC freeze pressure", badge: "Eco" },
    { id: "overclocked", name: "Overclocked Speedster", description: "High FPS canvas render with aggressive memory sweep", badge: "Overclock" },
  ],
  "clinical-chaos": [
    { id: "sdtm-pack", name: "Standard SDTM Domain Pack", description: "DM, VS, AE, LB regulatory mapping toolkit", badge: "Compliance" },
    { id: "fast-track", name: "Fast-Track Audit Kit", description: "Accelerated electronic signatures and rapid CDISC validation", badge: "Fast-Track" },
    { id: "fda-priority", name: "FDA Priority Pass", description: "Reduced auditor stress accumulation rate", badge: "Priority" },
  ],
  "retro-labyrinth": [
    { id: "debugger", name: "Standard Debugger", description: "Balanced maze traversal & developer weapon kit", badge: "Debug" },
    { id: "npm-armor", name: "npm install Armor", description: "Increased resistance against codebase bugs and corrupt nodes", badge: "Defense" },
    { id: "git-force", name: "git push --force Laser", description: "High-damage wall-shattering attacks against boss enemies", badge: "Force" },
  ],
  "default": [
    { id: "standard", name: "Standard Loadout", description: "Balanced starting parameters for arcade gameplay", badge: "Standard" },
    { id: "speedrun", name: "Speedrunner Kit", description: "Aggressive options for rapid playthroughs and high scores", badge: "Speed" },
    { id: "defense", name: "Defender Shield Pack", description: "Enhanced survival parameters and safety margins", badge: "Defense" },
  ]
};
...
export const BEZEL_STYLE_OPTIONS: { id: BezelStyle; label: string; desc: string; previewClass: string }[] = [
  ...
  { id: "minimal", label: "Dark Minimal", desc: "Sleek obsidian matte finish", previewClass: "border-zinc-700 bg-zinc-900/60 text-zinc-300" },
];
...
export function getSavedSetupConfig(gameId: string): GameSetupConfig {
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        difficulty: ["casual", "normal", "hard"].includes(parsed.difficulty) ? parsed.difficulty : defaultConfig.difficulty,
        loadout: parsed.loadout && loadouts.some(l => l.id === parsed.loadout) ? parsed.loadout : defaultConfig.loadout,
        screenShake: ["none", "subtle", "high"].includes(parsed.screenShake) ? parsed.screenShake : defaultConfig.screenShake,
        crtFilter: ["off", "soft", "arcade", "scanlines"].includes(parsed.crtFilter) ? parsed.crtFilter : defaultConfig.crtFilter,
        bezelStyle: ["classic", "neon", "woodgrain", "minimal"].includes(parsed.bezelStyle) ? parsed.bezelStyle : defaultConfig.bezelStyle,
      };
    }
  } catch {
...
export const PreGameSetupWizard: React.FC<PreGameSetupWizardProps> = ({
  ...
  const [config, setConfig] = useState<GameSetupConfig>(() => getSavedSetupConfig(gameId));
  ...
  useEffect(() => {
    if (isOpen) {
      announce(`Pre-Game Setup Wizard launched for ${gameTitle}. Step 1 of 3: Gameplay Configuration.`, "polite");
    }
  ...
  const handleStartGame = () => {
    ...
    playSubmit();
    announce("Pre-Game Setup confirmed. Starting active gameplay!", "assertive");
    onComplete(config);
  };

  // Find readable labels for summary preview
  const currentLoadoutObj = loadoutOptions.find((o) => o.id === config.loadout) || loadoutOptions[0];
  const currentDiffObj = DIFFICULTY_OPTIONS.find((d) => d.id === config.difficulty) || DIFFICULTY_OPTIONS[1];
  const currentShakeObj = SCREEN_SHAKE_OPTIONS.find((s) => s.id === config.screenShake) || SCREEN_SHAKE_OPTIONS[1];
  const currentCRTObj = CRT_FILTER_OPTIONS.find((c) => c.id === config.crtFilter) || CRT_FILTER_OPTIONS[1];
  const currentBezelObj = BEZEL_STYLE_OPTIONS.find((b) => b.id === config.bezelStyle) || BEZEL_STYLE_OPTIONS[0];
    ...
    >
      <div
        ...
        className={`w-full max-w-xl max-h-[92%] bg-zinc-950 border-2 border-amber-500/40 rounded-2xl p-4 sm:p-6 shadow-[0_0_50px_rgba(245,158,11,0.2)] flex flex-col justify-between overflow-y-auto text-white font-mono ${
          isCircularDisplay ? "rounded-full aspect-square justify-center p-6 text-[11px]" : ""
        }`}
      >
        ...
        <div>
          <div className="flex items-center justify-between border-b border-zinc-800 pb-3 mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 id="wizard-title" className="text-sm sm:text-base font-extrabold uppercase tracking-wider text-amber-400">
            ...
            <div className="flex items-center gap-1.5">
              {[1, 2, 3].map((step) => (
                ...
                >
                  {currentStep > step ? <IconCheck className="w-3.5 h-3.5" /> : step}
                </button>
              ))}
            </div>
          </div>
          ...
          {currentStep === 1 && (
            <div className="space-y-4 animate-in fade-in duration-150" data-testid="wizard-step-1">
                  ...
                  {DIFFICULTY_OPTIONS.map((opt) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {config.difficulty === opt.id && <IconCheck className="w-3.5 h-3.5 text-amber-400" />}
                      </div>
                      <p className="text-[9px] text-zinc-400 leading-tight hidden sm:block">{opt.desc}</p>
                    </button>
                  ))}
                </div>
              </div>
                  ...
                  {loadoutOptions.map((opt) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-zinc-400 leading-tight">{opt.description}</p>
                      </div>
                      {config.loadout === opt.id && <IconCheck className="w-4 h-4 text-amber-400 shrink-0" />}
                    </button>
                  ))}
                </div>
              </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-4 animate-in fade-in duration-150" data-testid="wizard-step-2">
              <div className="flex items-center gap-1.5 text-xs text-amber-300 font-bold uppercase tracking-wider">
                  ...
                  {CRT_FILTER_OPTIONS.map((opt) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-0.5">
                        ...
                        {config.crtFilter === opt.id && <IconCheck className="w-3.5 h-3.5 text-amber-400" />}
                      </div>
                      <p className="text-[9px] text-zinc-400 leading-tight hidden sm:block">{opt.desc}</p>
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {BEZEL_STYLE_OPTIONS.map((opt) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="font-bold text-xs text-white">{opt.label}</span>
                        {config.bezelStyle === opt.id && <IconCheck className="w-3.5 h-3.5 text-amber-400" />}
                      </div>
                      <p className="text-[9px] text-zinc-400 leading-tight hidden sm:block">{opt.desc}</p>
                    </button>
                  ))}
          ...
          {currentStep === 3 && (
            <div className="space-y-4 animate-in fade-in duration-150" data-testid="wizard-step-3">
              <div className="flex items-center gap-1.5 text-xs text-amber-300 font-bold uppercase tracking-wider">
              ...
              <div className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-3.5 sm:p-4 space-y-2.5 text-xs">
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-400 uppercase text-[10px]">Title:</span>
                  <span className="font-bold text-amber-400">{gameTitle}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-400 uppercase text-[10px]">Difficulty:</span>
                  <span className="font-bold text-white capitalize">{currentDiffObj.label}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-400 uppercase text-[10px]">Loadout:</span>
                  <span className="font-bold text-white text-right">{currentLoadoutObj.name}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-400 uppercase text-[10px]">Screen Shake:</span>
                  <span className="font-bold text-white">{currentShakeObj.label}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-400 uppercase text-[10px]">CRT Filter:</span>
                  <span className="font-bold text-white">{currentCRTObj.label}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-400 uppercase text-[10px]">Cabinet Bezel:</span>
                  <span className="font-bold text-white">{currentBezelObj.label}</span>
                </div>
              </div>
            </div>
          )}
