...
export const RetroLabyrinthClient: React.FC = () => {
  const [config, setConfig] = useState<RetroLabyrinthConfig | null>(null);
  ...
  return (
    <main className="min-h-screen bg-black text-white pt-28 pb-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Game Container */}
        <div className="rounded-3xl border border-zinc-800 bg-zinc-950/90 p-4 sm:p-6 shadow-[0_0_50px_rgba(244,63,94,0.1)] flex flex-col items-center justify-center min-h-[400px]">
          {config ? (
            <RetroLabyrinth isMounted={true} config={config} />
          ) : (
            <div className="text-center space-y-4 max-w-md mx-auto py-8 font-mono">
              <IconDeviceGamepad2 className="w-16 h-16 text-rose-500 mx-auto animate-pulse" />
              ...
              <button
                ...
                className="px-6 py-3 rounded-xl bg-rose-500 hover:bg-rose-400 text-black font-bold text-xs shadow-[0_0_15px_rgba(244,63,94,0.3)] transition-all uppercase"
              >
        ...
        <GameOnboardingWizard
          ...
          onComplete={(newConfig) => {
            setConfig(newConfig as RetroLabyrinthConfig);
            setWizardOpen(false);
          }}
        />
