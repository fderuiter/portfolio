...
const REALITY_CONTENT: Record<string, string> = {
  ...
  "polyglot-tsp": "Benchmarking Traveling Salesman Problem solvers across 50+ languages revealed stark runtime differences and memory overheads. Managing toolchain matrices and index offset conversions across array-oriented, functional, and hardware description languages required strict automated verification harnesses.",
  "oxidizemath": "Solving the 'two-language problem' in numerical computing required rigorous compile-time LaTeX macro verification. Initial double-buffering implementations caused transient frame drops during high-density PDE grid steps, resolved through zero-copy atomic state swaps.",
  ...
  "sortify": "Air-gapped document classification required local ONNX vector embeddings and sparse TF-IDF clustering. Ensuring atomic file operations across device boundaries necessitated shadow staging, journaled rollback recovery, and encrypted SQLCipher metadata caching.",
  "lambda-wave": "Bridging a pure Haskell DSP pipeline with lock-free C++ ring buffers required zero-copy FFI memory alignment and strict hardware timestamping. Garbage collector pauses during high-rate UART frame ingestion were eliminated by isolating memory mutation in C++ lock-free ring buffers while enforcing safety invariants via monotonic tokens."
};
