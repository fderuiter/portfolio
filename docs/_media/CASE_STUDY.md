...
- **Problem Solved**: Decodes obfuscated binary payloads and compressed QR code strings produced by bioelectrical impedance analysis (BIA) body composition monitors without relying on proprietary cloud APIs or closed vendor SDKs.
- **Core Technical Highlight**: Reverse-engineered payload decoding engine utilizing `data.py::decode_digits`, `mapping.py::_test_slice`, and differential mutation testing (`Differential Mutation Oracle`) to extract body fat percentage, Skeletal Muscle Index (SMI), visceral fat level, and total body water.
- **Key Metrics & Benchmarks**:
  - **100% Offline Parsing**: Zero network calls required for payload decoding.
  - **Sub-millisecond Processing**: Unpacks payload bits and maps clinical parameters instantaneously.

```mermaid
flowchart TD
    A[InBody QR Image / Raw Text Payload] --> B[Base64 / Custom Digit Unpacker]
    B --> C[data.py::decode_digits]
    C --> D[mapping.py::_test_slice]
    D --> E[Clinical Parameter Map: SMI, Fat%, TBW]
    E --> F[Differential Mutation Oracle Verification]
```

### Tags & Keywords
reverse-engineering, python, biomedical-data, qr-decoder, data-parsing
