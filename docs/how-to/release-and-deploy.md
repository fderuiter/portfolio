...
Last reconciled: 2026-09-24. Manual Production releases run through
...
The current GitHub Actions settings have no repository or environment secrets
and no Actions variables. CI uses only its disposable local Postgres service;
its literal `postgres` credentials are not production credentials. The old
`production-release` environment is not referenced by a workflow. Keep the
Protection Bypass credential managed in Vercel; `playwright.config.ts` reads
`VERCEL_AUTOMATION_BYPASS_SECRET` only when it is explicitly supplied to a
manual deployed-site probe, and GitHub Actions does not supply it.
