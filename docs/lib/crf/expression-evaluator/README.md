[**fderuiter-portfolio**](../../../README.md)

***

[fderuiter-portfolio](../../../modules.md) / lib/crf/expression-evaluator

# lib/crf/expression-evaluator

## Classes

- [ExpressionEvaluator](classes/ExpressionEvaluator.md)

## Interfaces

- [Token](interfaces/Token.md)

## Type Aliases

- [TokenType](type-aliases/TokenType.md)

## Functions

- [calculateBazettQTc](functions/calculateBazettQTc.md)
- [calculateBMI](functions/calculateBMI.md)
- [calculateCockcroftGaultCrCl](functions/calculateCockcroftGaultCrCl.md)
- [calculateDuboisBSA](functions/calculateDuboisBSA.md)
- [calculateFridericiaQTc](functions/calculateFridericiaQTc.md)
- [calculateMostellerBSA](functions/calculateMostellerBSA.md)
- [calculateRecistSldChange](functions/calculateRecistSldChange.md)
- [evaluateCondition](functions/evaluateCondition.md)
- [evaluateFormula](functions/evaluateFormula.md)
- [evaluateRule](functions/evaluateRule.md)
- [isSingleFormRule](functions/isSingleFormRule.md)
