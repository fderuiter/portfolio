[**fderuiter-portfolio**](../../../../README.md)

***

[fderuiter-portfolio](../../../../modules.md) / [lib/dx/doctor](../README.md) / checkNavbarHierarchy

# Function: checkNavbarHierarchy()

> **checkNavbarHierarchy**(`root`): [`DiagnosticCheckResult`](../interfaces/DiagnosticCheckResult.md)

Defined in: [lib/dx/doctor.ts:152](https://github.com/fderuiter/portfolio/blob/main/lib/dx/doctor.ts#L152)

Check Layout and Navbar Invariant (AGENTS.md Invariant #2).
No secondary Navbar components allowed outside app/layout.tsx.

## Parameters

### root

`string`

## Returns

[`DiagnosticCheckResult`](../interfaces/DiagnosticCheckResult.md)
