[**fderuiter-portfolio**](../../../README.md)

***

[fderuiter-portfolio](../../../modules.md) / [lib/graphics-engine](../README.md) / mapDataToCoordinates

# Function: mapDataToCoordinates()

> **mapDataToCoordinates**(`data`, `width`, `height`, `padding`, `minVal?`, `maxVal?`): [`Point2D`](../type-aliases/Point2D.md)[]

Defined in: [lib/graphics-engine.ts:226](https://github.com/fderuiter/portfolio/blob/main/lib/graphics-engine.ts#L226)

Maps a list of 1D data values to 2D Cartesian coordinates within a specified bounding box.
Takes min/max overrides or computes them from data.

## Parameters

### data

`number`[]

### width

`number`

### height

`number`

### padding

`number`

### minVal?

`number`

### maxVal?

`number`

## Returns

[`Point2D`](../type-aliases/Point2D.md)[]
