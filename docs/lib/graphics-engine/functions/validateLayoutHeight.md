[**fderuiter-portfolio**](../../../README.md)

***

[fderuiter-portfolio](../../../modules.md) / [lib/graphics-engine](../README.md) / validateLayoutHeight

# Function: validateLayoutHeight()

> **validateLayoutHeight**(`calculated`, `actual`, `contextMessage?`): `void`

Defined in: [lib/graphics-engine.ts:211](https://github.com/fderuiter/portfolio/blob/main/lib/graphics-engine.ts#L211)

Expose a layout validation utility that warns in non-production environments
when calculated layout height and actual physical DOM measurement differs by more than 2px.

## Parameters

### calculated

`number`

### actual

`number`

### contextMessage?

`string`

## Returns

`void`
