[**fderuiter-portfolio**](../../../README.md)

***

[fderuiter-portfolio](../../../modules.md) / lib/clinical-trial-chaos/engine

# lib/clinical-trial-chaos/engine

## Classes

- [ClinicalTrialChaosEngine](classes/ClinicalTrialChaosEngine.md)

## Interfaces

- [ClinicalTrialChaosSnapshot](interfaces/ClinicalTrialChaosSnapshot.md)
- [ClinicalTrialChaosState](interfaces/ClinicalTrialChaosState.md)

## Functions

- [calculateSubmissionPoints](functions/calculateSubmissionPoints.md)
- [chargePowerUps](functions/chargePowerUps.md)
- [createAuditLogEntry](functions/createAuditLogEntry.md)
- [createInitialAuditorState](functions/createInitialAuditorState.md)
- [createInitialPowerUpInventory](functions/createInitialPowerUpInventory.md)
- [createInitialScoreState](functions/createInitialScoreState.md)
- [exportToCDISCODMXML](functions/exportToCDISCODMXML.md)
- [exportToSDTMCSV](functions/exportToSDTMCSV.md)
- [fixObservation](functions/fixObservation.md)
- [formatAuditTimestamp](functions/formatAuditTimestamp.md)
- [generateBIMOReport](functions/generateBIMOReport.md)
- [generateSDTMDataset](functions/generateSDTMDataset.md)
- [getRoutingReadiness](functions/getRoutingReadiness.md)
- [getSubmissionMode](functions/getSubmissionMode.md)
- [isSubjectFullyCompliant](functions/isSubjectFullyCompliant.md)
- [scrambleStations](functions/scrambleStations.md)
- [tickAuditor](functions/tickAuditor.md)
- [tickPowerUps](functions/tickPowerUps.md)
- [tickSubjectTimers](functions/tickSubjectTimers.md)
- [triggerRandomAmendment](functions/triggerRandomAmendment.md)
- [validateObservationChoice](functions/validateObservationChoice.md)
- [verify21CFRSubmission](functions/verify21CFRSubmission.md)
