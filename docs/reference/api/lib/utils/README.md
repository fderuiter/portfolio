[**fderuiter-portfolio**](../../README.md)

***

[fderuiter-portfolio](../../modules.md) / lib/utils

# lib/utils

## Interfaces

- [EscapeXmlOptions](interfaces/EscapeXmlOptions.md)

## Variables

- [generateRandomId](variables/generateRandomId.md)

## Functions

- [cn](functions/cn.md)
- [escapeXml](functions/escapeXml.md)
- [formatDisplayDate](functions/formatDisplayDate.md)
- [formatIsoDate](functions/formatIsoDate.md)
- [formatRelativeTime](functions/formatRelativeTime.md)
- [generateId](functions/generateId.md)
- [getAnonymousDeviceHash](functions/getAnonymousDeviceHash.md)
- [getClientIp](functions/getClientIp.md)
- [hexToRgba](functions/hexToRgba.md)
- [isValidIsoDate](functions/isValidIsoDate.md)

## References

### cloneDeep

Re-exports [cloneDeep](clone/functions/cloneDeep.md)

***

### RequestOrHeaders

Re-exports [RequestOrHeaders](../services/privacy-service/type-aliases/RequestOrHeaders.md)
