...
export function computeFormHealthMetrics(form: CRFForm): FormHealthMetrics {
  ...
  const mandatoryFields = allFields.filter((f) => f.required).length;
  const codelistsAttached = allFields.filter((f) => Boolean(f.codelistId || (f.customOptions && f.customOptions.length > 0))).length;
  ...
  const presentVarNames = new Set(allFields.map((f) => f.variableName.toUpperCase()));
