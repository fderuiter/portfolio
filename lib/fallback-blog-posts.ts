...
<h3>The one I caused</h3>
<p>Rehearsing a database restore — a task that existed precisely because it had never been done — I called the provider's restore function and it cut production over. Not a failure of the tool: the parameter controlling that behaviour defaults to "yes, promote this", and the documentation said so. I read it and proceeded anyway.</p>
<p>No data was lost, and I verified that rather than assumed it: identical schema and content fingerprints before and after, and the newest write in the database predated the restore point by thirty-four days. The rollback was the instructive part. Reversing the designation did not move the compute endpoint with it, and the provider refuses both to delete a root branch's endpoint and to add a second one to an occupied branch. The operation was not symmetric, and nothing said so in advance.</p>
<p>The rehearsal was worth more for that than for the number it produced. A recovery procedure nobody has executed is a hypothesis. This one had a sixty-four second recovery time and a footgun in the second step, and only one of those was discoverable from the documentation.</p>
