...
// --- SVG Spline & Coordinate Projections (Browser-Free) ---
...
// --- Shared Coordinate & Touch Input Normalization ---
 ...
 */
export function extractClientCoordinates(
  e: {
    clientX?: number;
    clientY?: number;
    touches?: Array<{ clientX: number; clientY: number }> | { [key: number]: { clientX: number; clientY: number }; length: number };
    changedTouches?: Array<{ clientX: number; clientY: number }> | { [key: number]: { clientX: number; clientY: number }; length: number };
  }
): Point2D | null {
  if (!e) return null;
 ...
 */
export function getCanvasEventCoordinates(
  e: {
    clientX?: number;
    clientY?: number;
    touches?: Array<{ clientX: number; clientY: number }> | { [key: number]: { clientX: number; clientY: number }; length: number };
  ...
  return normalizeCanvasCoordinates(
    coords.x,
    coords.y,
    rect,
 ...
 */
export class TouchMouseDeduplicator {
   ...
   */
  recordTouch(timestamp: number = Date.now()): void {
   ...
   */
  shouldSuppressMouseEvent(now: number = Date.now()): boolean {
    return now - this.lastTouchTimestamp < this.thresholdMs;
  }
...
/**
 * Global singleton touch/mouse deduplicator instance for utility usage.
 */
export const globalTouchDeduplicator = new TouchMouseDeduplicator(500);

// --- 2D Physics & Boundary Checks (Browser-Free) ---
