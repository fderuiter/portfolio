...
export type AvalancheTriggerType =
  "natural" | "skier" | "explosive" | "cornice_fall" | "snowmobile";

export type SnowpackType =
  "slab" | "loose_powder" | "wet_slide" | "deep_persistent";

export interface AvalancheEventOptions {
  triggerType: AvalancheTriggerType;
  dangerLevel: 1 | 2 | 3 | 4 | 5; // 1: Low, 2: Moderate, 3: Considerable, 4: High, 5: Extreme
  slopeAngleDegrees?: number;
  snowpackType?: SnowpackType;
  location?: string;
  sector?: string;
...
/**
 * Generates an avalanche hazard/trigger event with danger rating and severity assignment.
 */
export function generateAvalancheTriggerEvent(
  options: AvalancheEventOptions
): PatrolEvent {
  const {
    triggerType,
    dangerLevel,
    slopeAngleDegrees = 35,
    snowpackType = "slab",
    location = "The Back Bowl / Belle Creek Headwall",
    sector = "Back Bowl",
    aspect = "N",
  } = options;
  ...
  if (
    ...
    (dangerLevel === 3 &&
      slopeAngleDegrees >= 35 &&
      (triggerType === "skier" || triggerType === "natural")) ||
    (snowpackType === "deep_persistent" && slopeAngleDegrees >= 38)
  ) {
    severity = "critical";
  } else if (
    dangerLevel >= 2 ||
  ...
  const dangerLabel = dangerLabels[dangerLevel] || "Unknown";
  const title = `Avalanche Trigger Alert: ${dangerLabel} Danger (Level ${dangerLevel})`;
  const description = `${triggerType.toUpperCase()} trigger on ${slopeAngleDegrees}° ${aspect}-aspect slope at ${location}. Snowpack: ${snowpackType}.`;

  return {
    id: `avalanche-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    timestamp: Date.now(),
    scenarioId: "general",
    action: "AVALANCHE_TRIGGER",
    type: "AVALANCHE_TRIGGER",
    title,
    description,
    severity,
    context: {
      ...
      sector,
      aspect,
      closureRecommended: dangerLevel >= 3 || severity === "critical",
    },
    payload: {
      dangerLevel,
      slopeAngleDegrees,
