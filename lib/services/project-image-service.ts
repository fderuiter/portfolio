...
// Global in-memory storage backing media asset references for serverless / dev environments
const globalMediaStorage = new Map<string, MediaAssetRecord>();
...
export function validateImageMagicBytes(
  ...
  if (normalizedMime === "image/svg+xml") {
    const snippet = buffer
      ...
      .toLowerCase();
    const hasSvgTag = snippet.includes("<svg") || snippet.includes("<?xml");
    const hasScriptTag = snippet.includes("<script");
...
export class ProjectImageService {
   ...
   */
  static saveMediaAsset(
    key: string,
    buffer: Buffer,
    contentType: string
  ): string {
    globalMediaStorage.set(key, {
      buffer,
      contentType,
      createdAt: new Date(),
    });
   ...
   */
  static getMediaAsset(key: string): MediaAssetRecord | null {
    return globalMediaStorage.get(key) || null;
  }

  /**
   * Deletes a media asset from storage by key.
   */
  static deleteMediaAsset(key: string): boolean {
    return globalMediaStorage.delete(key);
  }
  ...
  static async uploadProjectImage(
    ...
    try {
      // 4. Atomic database persistence and cache eviction
      await CaseStudyService.updateCaseStudyImage(slug, assetUrl);
      return { hero_image_url: assetUrl, key };
    } catch (error) {
      // 5. On failure: Clean up newly created asset and ensure prior asset is preserved
      ProjectImageService.deleteMediaAsset(key);
      if (existing) {
        try {
