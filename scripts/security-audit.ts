...
export function parseIgnoreRules(data: unknown, now: Date = new Date()): ParsedIgnoreRule[] {
  ...
  const validateAndAdd = (
    ...
    if (createdDate && expDate.getTime() > createdDate.getTime() + ninetyDaysMs) {
      rules.push({
        advisory,
        package: pkg || undefined,
        expiresAt,
  ...
  if (Array.isArray(data)) {
    for (const item of data) {
      if (typeof item === "string") {
        ...
        const advisory = String(obj.advisory || obj.advisoryId || obj.cve || obj.ghsa || obj.id || "");
        ...
        const createdAt = obj.createdAt || obj.created ? String(obj.createdAt || obj.created) : undefined;
    ...
    for (const [key, val] of Object.entries(data as Record<string, unknown>)) {
      if (val && typeof val === "object") {
        ...
        const advisory = String(obj.advisory || obj.advisoryId || obj.cve || obj.ghsa || obj.id || key);
        const pkg = String(obj.package || obj.name || "");
        const expiresAt = String(obj.expiresAt || obj.expires || "");
        const reason = String(obj.reason || obj.justification || "");
        const createdAt = obj.createdAt || obj.created ? String(obj.createdAt || obj.created) : undefined;
...
export function getAdvisoryIdentifiers(adv: Advisory): string[] {
  ...
  const rawId = anyAdv.id || anyAdv.advisoryId || anyAdv.advisory || anyAdv.ghsa || anyAdv.ghsaId || anyAdv.cve;
  if (rawId) {
  ...
  if (adv.url) {
    ...
    if (advNumMatches) advNumMatches.forEach(m => ids.add(m.replace("advisories/", "")));
  }
  ...
  if (Array.isArray(vuln.via)) {
    for (const item of vuln.via) {
      if (typeof item === "object" && item !== null) {
        ...
        const nested = collectAdvisoriesForVulnerability(item, vulnerabilities, visited);
...
export function matchAdvisoryRule(rule: ParsedIgnoreRule, adv: Advisory, pkgName: string): boolean {
  ...
  if (adv.source && String(adv.source).toLowerCase().includes(ruleAdv)) return true;
...
export function runSecurityAudit(options: SecurityAuditOptions = {}): boolean {
  ...
  for (const [pkgName, info] of Object.entries(vulnerabilities)) {
    ...
    if (severity === "high" || severity === "critical") {
      if (isPretextRelated(pkgName, vuln)) {
        ...
        const advisories = collectAdvisoriesForVulnerability(pkgName, vulnerabilities);

        for (const adv of advisories) {
          const matchingRule = ignoreRules.find(r => matchAdvisoryRule(r, adv, pkgName));
          ...
          if (matchingRule) {
            ...
            unhandledVulnerabilities.push({ pkgName, info: vuln, advisory: adv });
          }
        }
      }
    }
  ...
  const uniqueUnhandled = unhandledVulnerabilities.filter(item => {
    const advId = item.advisory ? getAdvisoryIdentifiers(item.advisory)[0] || item.advisory.title || "N/A" : "N/A";
  ...
  if (uniqueUnhandled.length > 0) {
    ...
    for (const { pkgName, info, advisory } of uniqueUnhandled) {
      ...
      if (advisory) {
        const advId = getAdvisoryIdentifiers(advisory)[0] || advisory.title || "N/A";
        console.error(`  ${colors.bold}Advisory ID:${colors.reset} ${advId}`);
        console.error(`  ${colors.bold}Advisory Title:${colors.reset} ${advisory.title || "N/A"}`);
        console.error(`  ${colors.bold}Advisory URL:${colors.reset} ${advisory.url || "N/A"}`);
        console.error(`  ${colors.bold}Vulnerable Range:${colors.reset} ${advisory.range || "N/A"}`);
      }
      console.error("");
    }
    failed = true;
